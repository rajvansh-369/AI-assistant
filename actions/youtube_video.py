...
def _open_url(url: str) -> None:
    try:
        ...
        log.error(f"⚠️ open_url failed: {e}")
...
def _scrape_first_video_url(query: str) -> str | None:
    ...
    try:
        ...
        log.error(f"⚠️ scrape_first_video_url failed: {e}")
...
def _ask_for_url(prompt_text: str = "YouTube video URL:") -> str | None:
    try:
        ...
        log.error(f"⚠️ URL dialog failed: {e}")
        return None
...
def _get_transcript(video_id: str) -> str | None:
    ...
    try:
        ...
        log.error(f"⚠️ Transcript fetch failed: {e}")
        return None
...
def _save_summary(content: str, video_url: str) -> str:
    ...
    try:
        ...
        log.error(f"⚠️ Could not open text editor: {e}")
...
def _scrape_video_info(video_id: str) -> dict:
    ...
    try:
        ...
        log.error(f"⚠️ Info scrape failed: {e}")
        return {}
...
def _scrape_trending(region: str = "TR", max_results: int = 8) -> list[dict]:
    ...
    try:
        ...
        log.error(f"⚠️ Trending scrape failed: {e}")
        return []
...
def _handle_play(parameters: dict, player) -> str:
    ...
    log.info(f"🔍 Scraping first non-Shorts video for: {query}")

    video_url = _scrape_first_video_url(query)

    if video_url:
        log.info(f"▶️ Opening: {video_url}")
        _open_url(video_url)
        return f"Playing: {query}"

    log.error(f"⚠️ Scrape failed, opening filtered search page")
    fallback_url = (
        f"https://www.youtube.com/results"
        f"?search_query={quote_plus(query)}"
        f"&sp={_YT_VIDEO_FILTER}"
...
def youtube_video(
    ...
    log.info(f"▶️  Action: {action}  Params: {params}")
...
def youtube_video_tool(params: dict, ctx: ToolContext) -> str:
    r = youtube_video(parameters=params, response=None, player=ctx.ui)
    return r or "Done."
