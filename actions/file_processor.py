...
def _gemini_client():
    ...
    class _W:
        def generate_content(self, contents):
            budget.reserve()
            try:
                return _c.models.generate_content(model=budget.model("fast"),
                                                  contents=contents)
...
def file_processor_tool(params: dict, ctx: ToolContext) -> str:
    # The HUD drop zone is the usual source of the file, so an omitted
    # file_path means "the one on screen".
    if not params.get("file_path") and ctx.current_file:
        params["file_path"] = ctx.current_file
    r = file_processor(parameters=params, player=ctx.ui, speak=ctx.speak)
    return r or "Done."
