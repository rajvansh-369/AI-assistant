...
def _launch_windows(app_name: str) -> bool:
    ...
    try:
        ...
        log.error(f"Start Menu search failed: {e}")
...
def open_app(
    ...
    log.info(f"Launching: '{app_name}' → '{normalized}' ({_SYSTEM})")
...
def open_app_tool(params: dict, ctx: ToolContext) -> str:
    r = open_app(parameters=params, response=None, player=ctx.ui)
    return r or f"Opened {params.get('app_name')}."
