...
def volume_set(value: int):
    ...
    if _OS == "Windows":
        try:
            ...
            log.error(f"pycaw failed, using keypress fallback: {e}")
            pyautogui.press("volumemute")
            pyautogui.press("volumemute")
    elif _OS == "Darwin":
...
def brightness_up():
    if _OS == "Darwin":
        ...
        try:
            ...
            log.error(f"Brightness up failed on Windows: {e}")
...
def brightness_down():
    if _OS == "Darwin":
        ...
        try:
            ...
            log.error(f"Brightness down failed on Windows: {e}")
...
def sleep_display():
    if _OS == "Windows":
        try:
            ...
            log.error(f"sleep_display failed: {e}")
    elif _OS == "Darwin":
        subprocess.run(["pmset", "displaysleepnow"], capture_output=True)
    else:
...
def dark_mode():
    if _OS == "Darwin":
        ...
        try:
            ...
            log.error(f"dark_mode registry failed: {e}")
    else:
        try:
            result = subprocess.run(
            ...
            log.error(f"dark_mode Linux failed: {e}")
...
def toggle_wifi():
    if _OS == "Darwin":
        ...
        try:
            ...
            log.error(f"toggle_wifi Windows failed: {e}")
    else:
        try:
            ...
            log.error(f"toggle_wifi Linux failed: {e}")
...
- Return ONLY the JSON, no explanation, no markdown."""
    ...
    try:
        budget.reserve()
        resp = _client.models.generate_content(model=budget.model("lite"),
                                               contents=prompt)
...
def computer_settings(
    ...
    log.info(f"Action: {action}  Value: {value}  OS: {_OS}")
    if player:
    ...
    try:
        ...
        log.error(f"Action failed ({action}): {e}")
...
def computer_settings_tool(params: dict, ctx: ToolContext) -> str:
    r = computer_settings(parameters=params, response=None, player=ctx.ui)
    return r or "Done."
