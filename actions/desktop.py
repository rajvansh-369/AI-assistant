...
Task: {task}"""
    ...
    try:
        budget.reserve()
        response = _client.models.generate_content(model=budget.model("fast"),
                                                   contents=prompt)
...
def desktop_control(
    ...
    try:
        if action == "wallpaper":
            ...
            log.info(f"Asking Gemini: {actual_task}")
            if player:
...
def desktop_control_tool(params: dict, ctx: ToolContext) -> str:
    r = desktop_control(parameters=params, player=ctx.ui)
    return r or "Done."
