...
log = get_logger("devagent")


PROJECTS_DIR     = Path.home() / "Desktop" / "JarvisProjects"
MAX_FIX_ATTEMPTS = 5
# Resolved per call, not at import: the model depends on free/paid mode and on
# whether a quota cooldown is currently in effect.
MODEL_PLANNER = "fast"
MODEL_WRITER  = "fast"
def _get_model(tier: str):
    model_name = budget.model(tier)
...
Code for {file_path}:"""
    ...
    try:
        ...
        log.info(f"✅ Written: {file_path} ({len(code)} chars)")
        return code
...
def _install_dependencies(dependencies: list[str], project_dir: Path) -> str:
    ...
    for dep in dependencies:
        ...
        if result.returncode != 0:
            ...
            log.info(f"✓ Already installed: {pkg_name}")
...
def _open_vscode(project_dir: Path) -> bool:
    ...
    for cmd in vscode_candidates:
        try:
            ...
            time.sleep(1.5)
            log.info(f"💻 VSCode opened: {project_dir}")
            return True
        except Exception:
            continue
    return False
...
def _run_project(run_command: str, project_dir: Path, timeout: int = 30) -> str:
    log.info(f"🚀 Running: {run_command}")
    try:
...
def _try_auto_install(error_output: str, project_dir: Path) -> bool:
    ...
    log.info(f"🔧 Auto-installing missing package: {pkg}")
    try:
        result = subprocess.run(
...
Fixed code for {fix_path}:"""
        ...
        try:
            ...
            log.info(f"🔧 Fixed: {fix_path}")

        except Exception as e:
            if _is_rate_limit(e):
                raise RateLimitError(str(e))
            log.error(f"⚠️ Could not fix {fix_path}: {e}")
...
def dev_agent_tool(params: dict, ctx: ToolContext) -> str:
    r = dev_agent(parameters=params, player=ctx.ui, speak=ctx.speak)
    return r or "Done."
