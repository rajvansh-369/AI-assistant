...
def _parse_date(raw: str) -> str:
    ...
    try:
        ...
        log.error(f"⚠️ Gemini date parse failed: {e}")
    ...
    # Last resort: today
    log.error(f"⚠️ Could not parse date '{raw}' — using today.")
...
def _parse_flights_with_gemini(
    ...
    try:
        ...
        log.error(f"⚠️ Gemini parse failed: {e}")
        return []
...
def flight_finder_tool(params: dict, ctx: ToolContext) -> str:
    r = flight_finder(parameters=params, player=ctx.ui)
    return r or "Done."
