...
"""
    ...
    if result.returncode != 0:
        ...
        script_path.unlink(missing_ok=True)
        log.error(f"❌ launchctl: {result.stderr.strip()}")
        return ""
...
def _schedule_linux(target_dt: datetime, task_name: str,
    ...
    if shutil.which("systemd-run"):
        ...
        if result.returncode == 0:
            return task_name
        log.error(f"⚠️ systemd-run failed: {result.stderr.strip()}, trying 'at'")
    ...
    if shutil.which("at"):
        ...
        if result.returncode == 0:
            return task_name
        log.error(f"❌ at: {result.stderr.strip()}")
        return ""

    log.error("❌ Neither systemd-run nor at found on this Linux system.")
    return ""
...
def reminder(
    ...
    try:
        ...
        script_path.unlink(missing_ok=True)
        log.error(f"❌ Scheduling exception: {e}")
...
def reminder_tool(params: dict, ctx: ToolContext) -> str:
    r = reminder(parameters=params, response=None, player=ctx.ui)
