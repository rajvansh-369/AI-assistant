...
def _open_app(app_name: str) -> bool:
    ...
    try:
        ...
        log.error(f"⚠️ Could not open {app_name}: {e}")
        return False
...
def _open_browser_url(url: str) -> bool:
    ...
    try:
        ...
        log.error(f"⚠️ Could not open browser: {e}")
        return False
...
def send_message(
    ...
    log.info(f"📨 {platform} → {receiver}: {preview}")
    if player:
    ...
    log.error(f"{'✅' if 'sent' in result.lower() else '❌'} {result}")
    if player:
...
def send_message_tool(params: dict, ctx: ToolContext) -> str:
    r = send_message(parameters=params, response=None, player=ctx.ui, session_memory=None)
    return r or f"Message sent to {params.get('receiver')}."
