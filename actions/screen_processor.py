...
from core.settings import LIVE_MODEL, get_settings, save_settings
...
def _load_config() -> dict:
    """Raw config dict — kept for the cached camera-index lookup below."""
    s = get_settings()
    return {**s.extra,
...
def _save_config_key(key: str, value) -> None:
    try:
        ...
        log.error(f"⚠️  Could not save config key '{key}': {e}")


def _get_os() -> str:
    return get_settings().os_system.lower()
...
def _compress(img_bytes: bytes, source_format: str = "PNG") -> tuple[bytes, str]:
    ...
    try:
        ...
        log.error(f"⚠️  Image compress failed: {e}")
        return img_bytes, f"image/{source_format.lower()}"
...
def _detect_camera_index() -> int:
    ...
    for idx in range(6):
        if _probe_camera(idx, backend):
            ...
            return idx
        log.warning(f"⚠️  Camera index {idx}: no usable frame")

    log.warning("⚠️  No camera found — defaulting to index 0")
...
class _VisionSession:
    ...
    def analyze(self, image_bytes: bytes, mime_type: str, user_text: str) -> None:
        if not self._loop or not self._out_queue:
            log.warning("⚠️  Session not started — dropping request")
            return
        ...
        while True:
            try:
                ...
                for exc in eg.exceptions:
                    log.error(f"⚠️  Session error: {exc}")
            finally:
                self._session = None
    ...
    async def _send_loop(self) -> None:
        while True:
            ...
            if not self._session:
                log.warning("⚠️  No session — dropping image")
                continue
            try:
                ...
                log.info(f"📤 Sent {len(image_bytes):,} bytes — '{user_text[:60]}'")
            except Exception as e:
                log.error(f"⚠️  Send error: {e}")
...
def screen_process(
    ...
    if not user_text:
        log.warning("⚠️  No question provided — aborting")
        return False

    log.info(f"▶ angle={angle!r}  question='{user_text[:80]}'")

    try:
        _ensure_session(player=player)
    except Exception as e:
        log.error(f"❌ Could not start session: {e}")
        return False

    try:
        if angle == "camera":
            image_bytes, mime_type = _capture_camera()
            log.info(f"📷 Camera: {len(image_bytes):,} bytes")
            if player and hasattr(player, "start_camera_stream"):
                try:
                    player.start_camera_stream()
                except Exception as _e:
                    log.error(f"⚠️  Camera stream failed: {_e}")
            elif player and hasattr(player, "show_camera_frame"):
                try:
                    player.show_camera_frame(image_bytes)
                except Exception as _e:
                    log.error(f"⚠️  Camera preview failed: {_e}")
        else:
            image_bytes, mime_type = _capture_screen()
            log.info(f"🖥️  Screen: {len(image_bytes):,} bytes")
    except Exception as e:
...
def warmup_session(player=None) -> None:
    try:
        ...
        log.error(f"⚠️  Warmup failed: {e}")

if __name__ == "__main__":
    log.info("[TEST] screen_processor.py")
    print("=" * 52)
