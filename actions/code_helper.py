...
def _get_gemini(model: str | None = None):
    ...
    # Resolved per call — free mode and quota cooldowns both change the answer.
    model = model or budget.model("fast")
...
def _take_screenshot() -> Path | None:
    try:
        ...
        log.error(f"⚠️ Screenshot failed: {e}")
        return None
...
def _detect_intent(description: str, file_path: str, code: str) -> str:
    ...
    # Classification is a convenience, not a requirement — the structural
    # fallback below covers it, so free mode does not spend a request on it.
    if desc and budget.allows("optional_llm"):
        try:
            ctx = []
            if file_path:
            ...
            log.error(f"Intent classification failed ({e}) — structural fallback")
...
def _build(description, language, output_path, args, timeout, speak=None, player=None) -> str:
    ...
    for attempt in range(1, MAX_BUILD_ATTEMPTS + 1):
        log.info(f"🔄 Attempt {attempt}/{MAX_BUILD_ATTEMPTS}")
        if player:
        ...
        log.error(f"⚠️ Error on attempt {attempt}, fixing...")
        if player:
...
Optimized code:"""
    ...
    log.info(f"✅ Optimized: {save_path}")
...
def _screen_debug_action(description, file_path, player, speak=None) -> str:
    ...
    log.info("📸 Capturing screen for debug...")
    ...
    if file_path:
        ...
        if err:
            log.error(f"⚠️ Could not read file: {err}")
...
Be specific and actionable. If you see an error message, quote it exactly."""
        ...
        log.info(f"✅ Screen analysis complete")
        ...
        if file_path and file_content:
            ...
            if code_match:
                ...
                log.info(f"✅ Fixed code saved: {file_path}")
...
def code_helper(
    ...
    if action == "auto":
        ...
        log.info(f"🤖 Auto-detected: {action}")
...
def code_helper_tool(params: dict, ctx: ToolContext) -> str:
    r = code_helper(parameters=params, player=ctx.ui, speak=ctx.speak)
    return r or "Done."
