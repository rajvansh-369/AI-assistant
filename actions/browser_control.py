...
def _real_profile_dir(browser: str) -> str:
    ...
    for p in candidates:
        if p.exists():
            log.info(f"✅ Real profile found for {browser}: {p}")
            return str(p)

    fallback = home / ".jarvis_profiles" / browser
    fallback.mkdir(parents=True, exist_ok=True)
    log.warning(f"⚠️  Real profile not found for {browser}, using: {fallback}")
    return str(fallback)
...
def _firefox_profile_dir() -> Optional[str]:
    ...
    if default_path and Path(default_path).exists():
        log.info(f"Firefox real profile: {default_path}")
        return default_path
    return None
            ...
            for hive in (winreg.HKEY_LOCAL_MACHINE, winreg.HKEY_CURRENT_USER):
                try:
                    ...
                    if exe and Path(exe).exists():
                        log.info(f"Opera found via registry: {exe}")
                        return exe
                except Exception:
                    continue
    except Exception:
...
def _resolve_browser(name: str) -> dict | None:
    ...
    if spec.get("special") == "opera_windows":
        ...
        if not exe:
            log.warning(f"⚠️  Opera executable not found on Windows.")
    ...
    if name:
        if _OS == "Darwin":
            ...
            if app:
                ...
                try:
                    ...
                    log.error(f"'open -a {app}' failed ({e}), trying binary…")
        ...
        if exe:
            try:
                ...
                log.error(f"Native launch failed for {name}: {e}")
        log.warning(f"'{name}' not found — falling back to default browser.")
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            ...
            try:
                ...
                log.error(f"Firefox real profile failed ({e}), using JARVIS profile")
        ...
        try:
            self._context = await engine_obj.launch_persistent_context(profile, **kwargs)
            self._page = await self._adopt_page()
            log.info(f"✅ Launched [{label}] profile={profile}")
            return
        except Exception as e:
            log.error(f"⚠️  Real profile failed for {label}: {e}")
        ...
        log.info(f"Retrying with JARVIS profile: {jarvis_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(jarvis_profile, **kwargs)
            self._page = await self._adopt_page()
            log.info(f"✅ Launched [{label}] with JARVIS profile "
    ...
    def _get_or_create(self, browser_name: str) -> _BrowserSession:
        with self._lock:
            if browser_name not in self._sessions:
                ...
                log.info(f"[Registry] New session: {browser_name}")
    ...
    try:
        ...
        if last:
            try:
                ...
                log.error(f"Could not resume last page ({last}): {e}")
...
def browser_control_tool(params: dict, ctx: ToolContext) -> str:
    r = browser_control(parameters=params, player=ctx.ui)
    return r or "Done."
