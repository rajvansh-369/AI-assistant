...
def _click_first_profile_by_screenshot() -> bool:
    ...
    try:
        ...
        if not win:
            log.warning("⚠️ Steam penceresi bulunamadı")
            return False
        ...
        if not colorful.any():
            log.warning("⚠️ Avatar rengi bulunamadı — tahminle tıklanıyor")
        ...
        log.info(f"🎯 Profil avatarı ({abs_x}, {abs_y}) — tıklanıyor")
        pyautogui.click(abs_x, abs_y)
        return True

    except ImportError as e:
        log.warning(f"⚠️ Eksik kütüphane: {e}")
        return False
    except Exception as e:
        log.warning(f"⚠️ Profil tespiti başarısız: {e}")
        return False


def _handle_steam_profile_selection() -> bool:
    log.info("🔍 Profil seçim dialogu kontrol ediliyor...")
    win = _get_steam_window_rect()
    if not win:
        return False
    ...
    try:
        ...
        if not is_small and white_pixels <= 100:
            log.info("ℹ️ Profil dialogu yok — Steam zaten giriş yapmış")
            return False
    except ImportError:
        pass
    except Exception:
    ...
    log.info("👤 Profil seçimi tespit edildi — ilk profile tıklanıyor")
        ...
        try:
            for ctrl in dialog.descendants(control_type=control_type):
                if target in ctrl.window_text().upper():
                    ctrl.click_input()
                    log.info(f"✅ Sürücü seçildi ({control_type}): {ctrl.window_text()}")
                    return True
        except Exception:
            continue
    try:
...
def _handle_install_dialog(game_name: str) -> str:
    ...
    log.info(f"🏆 Hedef sürücü: {drive_label} ({best_drive['free_gb']:.1f} GB boş)")
    ...
    try:
        ...
        log.warning(f"⚠️ pywinauto başarısız: {e}")
        return _handle_install_dialog_pyautogui(game_name, best_drive)
...
def _ensure_steam_running(steam_path: Path) -> bool:
    ...
    if not exe.exists():
        log.error(f"❌ Steam bulunamadı: {exe}")
        return False

    log.info("🚀 Steam başlatılıyor...")
    if is_mac():
    ...
    log.warning("⚠️ Steam başlatılamadı")
    return False
...
def _search_steam_appid(game_name: str) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        if items:
            best = items[0]
            log.info(f"🌐 Store API: {best['name']} ({best['id']})")
            return str(best["id"]), best["name"]
    except Exception as e:
        log.warning(f"⚠️ AppID arama başarısız: {e}")
...
def _install_steam_game(steam_path: Path, game_name: str = None,
    ...
    if not app_id and game_name:
        ...
        log.info(f"🔍 Kuruluyor: {game_name} (AppID: {app_id})")
...
def game_updater_tool(params: dict, ctx: ToolContext) -> str:
    r = game_updater(parameters=params, player=ctx.ui, speak=ctx.speak)
    return r or "Done."
