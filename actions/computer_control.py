...
log = get_logger("computercontrol")
...
def _screen_find(description: str) -> tuple[int, int] | None:
    ...
    if not api_key:
        log.warning("⚠️ No API key for screen_find")
        return None
    ...
    try:
        ...
        log.error(f"⚠️ screen_find failed: {e}")
    ...
    try:
        ...
        if action == "user_data":
            ...
            if not value:
                ...
                log.warning(f"⚠️ No '{field}' in memory, using random: {value}")
            return value
...
def computer_control_tool(params: dict, ctx: ToolContext) -> str:
    r = computer_control(parameters=params, player=ctx.ui)
    return r or "Done."
