...
from core.paths import BASE_DIR, CONFIG_DIR, CONFIG_PATH as API_FILE
from core.settings import get_settings


def _read_full_config() -> dict:
    """Full config dict, including keys core.settings does not model explicitly."""
    s = get_settings()
    return {**s.extra,
...
class MainWindow(QMainWindow):
    ...
    def _build_jarvis_icon(out_path: Path) -> bool:
        ...
        try:
            ...
            log.error(f"[Shortcut] ⚠️  Icon generation failed: {e}")
            return False
    ...
    def _build_quick_drawer(self) -> QWidget:
        ...
        self._mode_btn.setFont(QFont("Courier New", 7))
        self._mode_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self._mode_btn.clicked.connect(self._toggle_api_mode)
    ...
    def _toggle_api_mode(self):
        """Free ↔ paid. Free keeps every Gemini call inside a free key's quota."""
        from core import budget
        new_mode = budget.PAID if budget.is_free() else budget.FREE
        budget.set_mode(new_mode)
        self._update_mode_btn()
        if new_mode == budget.FREE:
            self._log.append_log(
                "SYS: FREE mode — DuckDuckGo for search, lite model, rate-limited. "
                "No quota errors."
            )
        else:
            self._log.append_log(
                "SYS: PAID mode — grounded Google search and the full model enabled. "
                "A free API key will hit 429 quota errors."
            )
    ...
    def _update_mode_btn(self):
        ...
        from core import budget
        if budget.is_free():
            self._mode_btn.setText("🆓  API MODE: FREE")
            ...
            self._mode_btn.setToolTip(
                "FREE — no grounded Google search (DuckDuckGo instead), lite model, "
                "max 8 Gemini requests/min. Stays inside a free API key's quota."
            )
        else:
            self._mode_btn.setText("💳  API MODE: PAID")
            self._mode_btn.setStyleSheet(f"""
                QPushButton {{
                ...
                QPushButton:hover {{ background: #201700; }}
            """)
            self._mode_btn.setToolTip(
                "PAID — grounded Google search and the full model. "
                "Needs a billed API key; a free key returns 429 quota errors."
            )
